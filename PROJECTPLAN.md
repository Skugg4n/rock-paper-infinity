...
- [ ] **"Win-ring" animation** — clarified in v1.10.0; shipped in v1.11.0; gated to first-3 wins in v1.11.2 to avoid going stale. Larger redesign still possible.
- [ ] **Enemy red triangle** — shipped in v1.11.0, removed in v1.11.2 because the triangle plus the slate ring made the result wrapper "stökigt och fult". Intent stays valid (red triangle as enemy signature, callback in Phase 3 WAR) but needs proper design — smaller, subtler, possibly outside the wrapper rather than under it. Picked up when there's design bandwidth.
- [ ] **Sims-style pedestrian movement (Phase 2)** — Jacks SimsSpel-inspired: small circles representing people moving between houses, factories, shops. Substantial new feature; canvas overlay or SVG. Defer to v1.12+ at least.
